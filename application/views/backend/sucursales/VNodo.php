		            ...
		            success: function(data){     
...

						{
...

							{
								...
								html += "<div class='wrapper' id='wrapper"+response.pedido[i]['id_pedido']+"' ><div class='list-group abc'><a href='#' class='list-group-item default comanda'><i class='fa fa-home'></i><b>ORDEN -  # "+response.pedido[i]['id_pedido']+response.pedido[i]['secuencia_orden']+"  ( " + response.pedido[i]['fechahora_pedido'] + " )</b> <i style='color:green;'>"+ "</i></a>";
...

								{
									...
									html += "<tr><td width='30%'>[ "+ response.pedido[i][j]['pedido_estado'] +" ] - "+response.pedido[i][j]['nombre_producto']+" / <br>"+"<img src='/uploaded/mod_productos/"+response.pedido[i][j]['image']+"' width='150px' height='225px' />";
									...
									html += "Estado : <select name='estado' class='form-control' id='estado"+response.pedido[i]['id_pedido']+"'>"+
												...
												"<?php foreach($estados as $est){ if($est->id_pedido_estado==3){ ?>"+
									...
									html +="Precio Grabado:<b>$ "+response.pedido[i][j]['precio_grabado']+"</b>"+
											...
											"<br>Cantidad :<b> "+ response.pedido[i][j]['cantidad'] +"</b><br><a  href='#' class='btn btn-success completar' id='"+response.pedido[i]['numero_mesa']+"' secuencia='"+response.pedido[i]['secuencia_orden']+"' pedido='"+response.pedido[i]['id_pedido']+"'>Completar</a>";
				...
				{					
...

					{
						...
						// Detalle Productos
									...
									html += "<tr><td width='30%'>[ "+ response.detalle[j]['pedido_estado'] +" ] - "+response.detalle[j]['nombre_producto']+" / " +"<img src='/uploaded/mod_productos/"+response.detalle[j]['image']+"' width='150px' height='' />";
									...
									html +="Precio Grabado:<b>$ "+response.detalle[i]['precio_grabado']+"</b>"+
											...
											"<br>Cantidad :<b> "+ response.detalle[i]['cantidad'] +"</b><br><a  href='#' class='btn btn-success completar' id='"+response.detalle[j]['numero_mesa']+"' secuencia='"+response.detalle[i]['secuencia_orden']+"' pedido='"+response.detalle[i]['id_pedido']+"'>Completar</a>";
