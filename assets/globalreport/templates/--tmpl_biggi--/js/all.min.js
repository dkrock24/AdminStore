...
H(function(){var a=window.external&&window.external.getUnityObject&&window.external.getUnityObject(1);if(a){var b={};b.name=document.querySelector("meta[itemprop=name]").content;b.iconUrl=document.querySelector("link[rel=apple-touch-icon]").href;u(b.name&&b.iconUrl);b.onInit=s();a.init(b)}$a.Gc=function(){var b=ab;a&&a.Notification.showNotification(b,"",m)};$a.uc=function(b,d,e){a&&(a.Launcher.setCount(d),b!==j&&b!==m?a.Launcher.setProgress(b):a.Launcher.clearProgress(),a.Launcher.setUrgent(e))};
$a.rc=function(b,d){a&&(a.Launcher.removeActions(),d&&(a.addAction("/Toggle Pause",function(){d.click()}),a.Launcher.addAction("Pause/Resume/Stop",function(){d.click()})),b.forEach(function(b){var c={2:"Countdown"};c[X]="Alarm Clock";c[1]="Stopwatch";a.addAction("/"+c[b.type],function(){b.onsubmit()});a.Launcher.addAction("Start "+c[b.type],function(){b.onsubmit()})}))}});
...
fb.push(function(){window.chrome&&(window.chrome.browserAction&&window.chrome.browserAction.setBadgeText)&&(Z.Db=function(a,b){window.chrome.browserAction.setBadgeText({text:A.f.X(0>a?0:1E3*a,"text")});var c=b===j||b===m?[0,0,255]:[parseInt(255*b,10),parseInt(255*(1-b),10),0],c="#"+c.map(function(a){a=a.toString(16);return 1===a.length?"0"+a:a}).reduce(function(a,b){return a+b});window.chrome.browserAction.setBadgeBackgroundColor({color:c})})});Z.B={};window.external&&(Z.zb=function(){try{window.external.msSiteModeActivate()}catch(a){}});
H(fb);
Y.state=function(a,b){db=b;Z.K={};var c={},d=[];d.push(function(){Z.B.ub&&(c.bb=function(){Z.B.ub(ab)})});d.push(function(){function b(){d||(d=document.getElementById("notify_sound"));if(!d)return l}var d;c.ic=function(){if(b())return l;try{var c=function(){d.currentTime=0;d.play()};c();var e=5;d.Hb||(d.addEventListener("ended",function(){(a.Da||e--)&&c()},n),d.Hb=l)}catch(f){return l}};c.xc=function(){b();try{d.pause()}catch(a){}}});d.push(function(){var b=window.AudioContext||window.webkitAudioContext;if(b){var d,
e,f;c.kc=function(){if(!d){e=new b;var c=e.sampleRate,x=2*Math.PI,q=e.createBuffer(1,88473.6,c),w=q.getChannelData(0);for(i=0;88473.6>i;++i){var r=i,B=88473.6/100;w[i]=(r>B?1-(r-B)/88473.6:r/B)*Math.sin(440*x*i/c)}d=e.createBufferSource(0);d.buffer=q;d.loop=l;d.looping=l;d.noteOn(e.currentTime)}d.connect(e.destination);f&&clearTimeout(f);a.Da||(f=setTimeout(function(){d.disconnect()},12E4))};c.zc=function(){d&&d.disconnect()}}});d.push(function(){if(!cb){var b={};a.style.position="relative";var d;
a.ta=function(){d=l;a.style.height="1px";a.style.width="1px";a.style.top="-10000px"};a.ta();a.show=function(){d=n;a.style.height="";a.style.width="";a.style.top=""};a.kd=function(){return d};a.gc=function(b){u(b.constructor===String);b=b.replace(" ","");a.Sb=b;var c=/youtube\.com.*(?:\?|&)v=([a-zA-Z0-9\-_]+)/.exec(b)||/youtu.be\/([a-zA-Z0-9\-_]+)/.exec(b);a.ja=c?c[1]:m;matches=/(?:\?|&)(?:start|t)=([^&#]*)/.exec(b);a.ia=matches?matches[1]:m;a.Da=/repeat|replay|loop/.test(b);a.yb=!a.ja?m:"http://www.youtube.com/embed/"+
a.ja+"?enablejsapi=1"+(a.ia?"&start="+a.ia:"")+"&origin=http://"+location.host+"&rel=0&controls=0&hd=0&showinfo=0&html5=1"};a.Bd=s();var e,f;b.Fa=function(b){if(!e){e=document.createElement(L()?"webview":"iframe");e.style.border="0";e.style.position="absolute";e.style.left="0";e.style.top="0";e.style.width="100%";e.style.height="100%";var c=document.createElement("div");c.style.position="absolute";c.style.left="0";c.style.top="0";c.style.width="100%";c.style.height="100%";c.style.cursor="pointer";
a.appendChild(e);a.appendChild(c)}b!=e.src&&(e.src=b);"WEBVIEW"===e.nodeName&&(f=b,setTimeout(function(){e.src!=f&&(e.src=f)},0))};var v;b.N=function(){if(!v){var a=e,b=function(){if(!fa){try{a.contentWindow&&a.contentWindow.postMessage(JSON.stringify({event:"listening",id:1}),'*')}catch(c){}setTimeout(b,300)}},c=function(){u(fa);u(a.src);u(a.contentWindow);callStack.forEach(function(b){var c=b[1];a.contentWindow.postMessage(JSON.stringify({event:"command",func:b[0],args:c?[c]:[],id:1}),'*')});
callStack=[]},d=function(a,b){callStack.push([a,b]);fa&&c()};if(typeof window.postMessage){var f={state:-4},g,h,p;window.addEventListener("message",function(b){if(b&&b.origin&&/youtube\.com$/.test(b['origin'])&&b.source===a.contentWindow&&b.data){b=JSON.parse(b.data);"onReady"===b.event&&(fa=l,c());if(b.info){var d=b.info.playerState;d!==j&&(d!==m&&d!==f.state)&&(f.state=d,f.jb&&f.jb(d),g&&(g(),g=j));var e;b.info.videoData&&(d=b.info.videoData.video_id,h!==d&&(h=d,e=l));d=b.info.videoBytesTotal;
...
a)};f.wc=function(){d("setVolume",100)};v=f}else v=n}return v};a.xd=s();var x,q;c.lc=function(){function c(){if(q)try{var a=b.N();a.play();a.Ec();a.wc()}catch(d){u(n)}}if(x||!a.yb)return l;b.Fa("about:blank");a.show();b.Fa(a.yb+"&autoplay=1");try{b.N()}catch(d){u(n)}try{var e=b.N();e.jb=function(b){0===b&&a.Da&&setTimeout(e.play,100)}}catch(f){u(n)}q=setTimeout(function(){if(q){var a;try{a=L()?-2===b.N().state:1===b.N().state}catch(c){}a||(x=l,setTimeout(function(){x=n},15E3),Z.K.stop(),Z.K.lb())}},
...
d=document.getElementById("pause"),e=document.getElementById("yt"),f=document.getElementById("alarmTime"),h=[];if(ea){var g=document.getElementById("time"),k=document.getElementById("allcontent"),p=document.getElementById("counter"),v=window.WinJS&&window.WinJS.UI&&window.WinJS.UI.processAll,x=H(function(){if(window.Oc){var a=[];(new MutationObserver(function(){setTimeout(function(){a.forEach(function(a){a()})},600)})).$b(document.documentElement,{attributes:l,Bb:["class"]});return a}}),q=[];h.push(function(){function a(){var c=
"#options"===location.hash;if(!!b!==!!c){document.documentElement.classList[c?"add":"remove"]("fullscreenOptions");var d={Wb:2,gb:b===j};x&&(d.D=x);ua(document.getElementById("settingsWrapper"),!c,d)}b=c}var b;ia(a);a()});h.push(function(){xa()&&(window.chrome.browserAction.onClicked.addListener(function(){d.click()}),(new MutationObserver(function(){window.chrome.browserAction.setPopup({popup:document.body.classList.contains("ringing")?"":"index.html"})})).$b(document.body,{attributes:l,Bb:["class"]}))});
...
(!window.chrome||!window.chrome.app||!window.chrome.app.isInstalled)&&!L()){var a=document.getElementById("install");a.style.display="";a.onclick=function(){window.chrome.webstore.install()}}});h.push(function(){if(e.wd)document.getElementById("goto").style.display="none";else{var a;sa("goto_url","http://youtu.be/PS5KAEgrtMA",function(b){e.gc(b);a||(setTimeout(s(),1600),a=l)})}});q.push(function(){function a(){I.get("disableNotification",function(a){b.checked=!(O.z()||a)})}if(O.Xa()){var b=document.getElementById("hNotiBox");
a();b.onchange=function(){I.get("disableNotification",function(b){b||O.z()?(O.ba()?O.ab&&alert(O.ab):O.z()&&O.ca(a),I.set("disableNotification")):I.set("disableNotification","true");a()})}}else document.getElementById("hNoti").style.display="none"});q.push(function(){document.getElementById("bg_url_container").appendChild(sa("bg_url","",ta()))});q.push(function(){v&&["settingsWrapper","hShare"].map(function(a){return document.getElementById(a)}).forEach(function(a){a.style.display="none"})});q.push(function(){var a=
{Pa:[d,f]};x&&(a.D=x);va(p,"f",a)});q.push(function(){!L()&&(!M()&&!v)&&setTimeout(function(){var a=document.createElement("iframe");a.src="http://brillcdn.appspot.com/sf/tt.html";a.id="files_iframe";a.style.display="none";document.body.appendChild(a);Ba("UA-5263303-6",l);a=document.createElement("script");a.type="text/javascript";a.async=l;a.src="https://apis.google.com/js/plusone.js";var b=document.getElementsByTagName("script")[0];b.parentNode.insertBefore(a,b);a.onload=function(){setTimeout(function(){document.getElementById("plusone_wrapper").style.opacity=
""},500)};var c=document.getElementById("hTweet");c.src="https://platform.twitter.com/widgets/tweet_button.html?url=http%3A%2F%2Fwww.timer-tab.com&text=Well%20Designed%20Timer%20Web%20App,%20made%20by%20@brillout&count=horizontal";c.onload=function(){c.style.opacity=""}},1600)});q.push(function(){var a=document.createElement("audio");a.id="notify_sound";a.style.display="none";[["mp3","mpeg"],["ogg","ogg"],["wav","wav"]].forEach(function(b){var c=a.appendChild(document.createElement("source"));c.type=
"audio/"+b[1];c.src="http://brillcdn.appspot.com/sf/ring."+b[0]});document.body.appendChild(a)});q.push(function(){function a(){qa(function(){var c=A.f.getTime(+new Date,document.documentElement.classList.contains("de")||document.documentElement.classList.contains("fr")?n:l);b!==c&&(b=c,g.innerHTML=c);window.setTimeout(a,1E3)})}var b;a()});var w=document.getElementById("stopwButton"),r=[].slice.call(b.getElementsByTagName("input")).concat([].slice.call(a.getElementsByTagName("input"))),B=r[1];q.push(function(){function b(c){c=
c||window.event;if(!E(c)){var d=c.charCode||c.which;48<=d&&57>=d?this.parentNode===a&&2<=this.value.length&&(this.value=""):32<=d&&c.preventDefault()}}function c(a){a=a||window.event;if(!E(a)){var b=G(a),d=b===(v?"down":"up")||"+"===b;if(b===(v?"up":"down")||"-"===b||d){var b=l,e=parseInt(this.value,10);e||(e=0);var d=e+(d?1:-1),e=this.getAttribute("min"),f=this.getAttribute("max");e&&f&&(parseInt(f,10)+1===d&&(b=l,d=e),parseInt(e,10)-1===d&&(b=l,d=f));b&&(a.preventDefault(),e&&(d=Math.max(e,d)),
f&&(d=Math.min(f,d)),this.value=d,H(this.oninput));this.selectionStart!==j&&(this.selectionStart=0,this.selectionEnd=this.value.length);if(b)return n}}}window.onkeydown=function(a){a=a||window.event;if(!E(a)){var b=pa(a).type;"text"===b||"url"===b||(b=G(a)," "===b&&a.preventDefault()," "===b||"p"===b?d.click():"esc"===b&&p.ha&&p.ha())}};for(var e=D().L?"keypress":"keydown",f=0;f<r.length;f++)(function(){"INPUT"===r[f].tagName&&(r[f].addEventListener("keypress",b,n),r[f].addEventListener(e,c,n));var a=
...
function(a){if(0!==a.detail)return a.stopImmediatePropagation(),a.preventDefault(),n});a.addEventListener("mousedown",function(){a.click()});a.addEventListener("touchstart",function(b){b.preventDefault();a.click()})});q.push(function(){N.get(function(a){var b,c;a&&"en"!==a&&document.documentElement.classList.add(a);"de"===a?(a="Adresse von ",b="YouTube Video",c="Bild"):("fr"===a?(a="adresse d'une ",b="video YouTube"):(a="address of ",b="YouTube video"),c="image");document.getElementById("feedback").href=
"http://www.google.com/moderator/#15/e=1fde25&t=1fde25.41&v=24";document.getElementById("promo").href="https://chrome.google.com/webstore/detail/facebook-unseen/dmdhkalcecemojegheiohcghkamlipof";[["goto_url",a+b],["bg_url",a+c]].forEach(function(a){var b=document.getElementById(a[0]);b&&b.setAttribute("placeholder",a[1])})})});q.push(function(){if(L()){document.documentElement.style.overflow="hidden";var a=Math.min(window.innerWidth/(k.scrollWidth+1),window.innerHeight/k.scrollHeight);document.documentElement.style.zoom=
a;za(arguments.callee,100,function(){document.documentElement.style.zoom=""})}});q.push(function(){function a(){window.clearTimeout(d);d=setTimeout(function(){b.forEach(function(a,b){a.style.height=c[b]});900<=window.innerWidth&&b.forEach(function(a){a.style.height=a.parentElement.scrollHeight+"px"})},300)}if(/MSIE/.test(navigator.userAgent)||window.opera){document.documentElement.classList.add("noIcons");var b=[document.getElementById("middletable").firstChild,document.getElementById("vertical"),
